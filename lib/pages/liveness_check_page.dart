...
import 'package:sample_liveness_app/blink_detector.dart';
...
class _LivenessCheckPageState extends State<LivenessCheckPage> {
    ...
    timer = Timer.periodic(Duration(seconds: 1), (timer) {
      ...
      if(_verificationTimeCounter > _verificationTimeoutInSecond){
        Fluttertoast.showToast(
          msg: "Failed : verification timeout(>${_verificationTimeoutInSecond}s)!.",
  ...
  bool _faceValidation(List<Face> faces){
    ...
    if(faces.length > 1){
      throw LivenessCheckException("Please stand alone, Make sure nobody behind you");
    }
    ...
    if(!isFaceFullyVisible){
      throw LivenessCheckException("Please keep your entire face on camera -_-");
    }
    // done ):
    return true;
  }
      ...
      if(_cameraFrameCaptured == null){
        ...
        WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
          ...
          Navigator.pushReplacement(
            context, 
           MaterialPageRoute(builder: (context) => KYCFacePage(cameraCaptured: _cameraFrameCaptured!),)
          );
        });
      }
    }else {
