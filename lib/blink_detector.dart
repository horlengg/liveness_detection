...
class BlinkDetector {
  ...
  bool detectBlink(Face face) {
    ...
    final eyesClosed = leftCurrent < closedThreshold && rightCurrent < closedThreshold;
    
    // Check if eyes are currently open
    final eyesOpen = leftCurrent > openThreshold && rightCurrent > openThreshold;
    ...
    if (!_isBlinking && eyesClosed) {
      ...
      if (
        (_lastBlinkTime == null || DateTime.now().difference(_lastBlinkTime!) > blinkCooldown) && 
